...
- **history** の各エントリの JSON 形状が、静的版の `pushHistory` と **完全一致しているかは未検証**。
- **stats** の勝敗・内訳の集計が、静的版の `game.stats` 更新と **厳密一致しているかは未検証**。
...
- **PWA 本対応**（manifest / service worker を React ビルドにどう載せるか）
- **manifest / service worker の React 版整理**（静的版の `manifest.json` / `service-worker.js` との関係）
- **GitHub Pages / Vite `base` 設定**（公開パスに合わせた設定）
- **既存静的版の置き換え**（ルートの `index.html` 差し替え等）
- **localStorage 完全互換の精査**（history / stats のフィールド単位）
- **stats 勝敗集計の厳密一致**
- **history 保存形式の完全一致**
- **高度な CPU AI**
- **カジノ細則の完全再現**
...
- localStorage 互換性チェック（settings は比較的揃えやすい。history / stats は要比較）
- stats / history の既存静的版との差分確認（フィールド・意味・更新タイミング）
