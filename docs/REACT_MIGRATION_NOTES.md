...
- **GitHub リポジトリ設定での Pages 有効化**（Source を GitHub Actions にする操作はローカルでは未実施）
...
- `react-ui/dist/` を **GitHub Pages のサイトルートとして配信**する（artifact の中身が `https://<user>.github.io/seven-stud-hilo-app/` にマッピングされる）
- ルートの静的版ファイルは **リポジトリ上は未削除**（配信は Actions 側の artifact が優先される）

### 9.2 GitHub Actions による Pages デプロイ（workflow）

- **ファイル**: `.github/workflows/deploy-react-pages.yml`
- **workflow 名**: `Deploy React to GitHub Pages`
- **トリガー**:
  - `main` ブランチへの `push`
  - `workflow_dispatch`（手動実行）
- **処理概要**:
  1. `actions/checkout`
  2. `actions/setup-node`（Node **22**、npm cache は `react-ui/package-lock.json`）
  3. `working-directory: react-ui` で `npm ci` → `npm run build`
  4. `actions/configure-pages`
  5. `actions/upload-pages-artifact` で **`react-ui/dist` のみ**をアップロード
  6. `actions/deploy-pages` で公開
- **permissions**: `contents: read`, `pages: write`, `id-token: write`
- **concurrency**: `pages-${{ github.workflow }}-${{ github.ref }}`（Pages デプロイの競合を抑制）

#### GitHub 側で必要な手動設定（初回）

1. リポジトリ **Settings → Pages**
2. **Build and deployment** の **Source** を **GitHub Actions** に変更
3. 初回デプロイ後、Actions の `deploy` ジョブが成功することを確認

#### 初回デプロイ後に確認する URL（例）

- アプリ: `https://yuusuke10000.github.io/seven-stud-hilo-app/`
- manifest: `https://yuusuke10000.github.io/seven-stud-hilo-app/manifest.webmanifest`
- Service Worker: `https://yuusuke10000.github.io/seven-stud-hilo-app/sw.js`
- アイコン: `https://yuusuke10000.github.io/seven-stud-hilo-app/icon.svg`

（ユーザー名・リポジトリ名が異なる場合は読み替える。）

#### PWA 確認（本番）

- DevTools → Application → **Manifest** / **Service Workers**
- オフラインで再読み込みできるか（初回はオンラインで SW 登録が必要）
- 更新が反映されない場合はキャッシュ・SW の更新タイミングを疑う（別端末でも再現するか）

#### ロールバック（方針）

- **workflow を無効化**する、または **`main` を前コミットに戻して再デプロイ**する
- リポジトリのルート静的版は **今回の workflow では変更しない**ため、緊急時は Pages の設定・デプロイ方法の切替も選択肢（運用次第）
...
- 2026-05-07: `main` push / `workflow_dispatch` で `react-ui/dist` を GitHub Pages に載せる `.github/workflows/deploy-react-pages.yml` を追加。手順・URL をセクション 9.2 に追記。
